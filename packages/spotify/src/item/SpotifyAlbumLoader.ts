...
export class SpotifyAlbumLoader extends Loader {
   ...
   */
  async load(manager: SpotifyManager, [ , id ]: RegExpExecArray): Promise<SpotifyAlbum> {
    const album = await manager.makeRequest<Spotify.Album>(`/albums/${id}`);
    return new SpotifyAlbum(manager, album, await SpotifyAlbumLoader.loadTracks(manager, album));
  }
   ...
   */
  private static async loadTracks(manager: SpotifyManager, album: Spotify.Album): Promise<SpotifyTrack[]> {
    ...
    while (next != null && !limit ? true : page < limit) {
      const {
        items,
        next: _next,
      } = await manager.makeRequest<Spotify.PagingObject<Spotify.Track>>(next!, false);
