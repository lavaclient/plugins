...
import { Manager, VoiceServer, VoiceState } from "lavaclient";
import { SpotifyPlugin } from "../dist";
...
client.ws.on("raw", async (pk: Payload) => {
  switch (pk.t) {
    case GatewayEvent.VoiceServerUpdate:
      await manager.serverUpdate(pk.d as VoiceServer);
      break;
    case GatewayEvent.VoiceStateUpdate:
      await manager.stateUpdate(pk.d as VoiceState);
      break;
  }
});
  ...
  switch (cmd.toLowerCase()) {
    ...
    case "play":
      ...
      if (manager.spotify.isSpotifyUrl(query)) {
        ...
        const results = await manager.search(`${/^(https?:\/\/)/im.test(query) ? "" : "ytsearch:"}${query}`);
